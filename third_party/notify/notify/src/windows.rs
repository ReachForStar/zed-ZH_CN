...
// 16KB 对高频变更目录（如大型仓库下的构建产物目录）过小，突发变更极易溢出；
// 扩到 64KB 降低溢出频率。溢出仍由 handle_event 的 ERROR_NOTIFY_ENUM_DIR 分支兜底。
...
unsafe extern "system" fn handle_event(
    ...
    match error_code {
        ...
        ERROR_NOTIFY_ENUM_DIR => {
            // 内核缓冲区溢出：自上次完成以来积压的变更超出缓冲区，部分事件
            // 丢失，但目录句柄仍然有效。重新发起读取并上报 rescan 事件让
            // 消费者全量对账，而不是静默 unwatch 导致永久丢失监听。
            log::warn!(
    ...
    // 部分 Windows 版本上缓冲区溢出表现为 ERROR_SUCCESS + 零字节（而非
    // ERROR_NOTIFY_ENUM_DIR）：积压的变更连一个条目都装不下。此时缓冲区为
    // 全零，直接解析会静默丢弃所有事件；按溢出处理，上报 rescan 让
    // 消费者全量对账。
