            ...
            h_flex()
                ...
                .child(
                    CopyButton::new(copy_button_id, code)
                        .tooltip_label(t!("agent_ui.thread_view.copy_code")),
                ),
        )
        .into_any_element()
}
...
impl ThreadView {
        ...
        if let Some(fallback_model) = acp_thread::refusal_fallback_model_from_meta(&state.meta) {
            return Some(
                Callout::new()
                    ...
                    .description(t!(
                        "agent_ui.thread_view.retrying_with",
    ...
    pub(crate) fn render_main_agent_awaiting_permission(
        ...
        let label: SharedString = if pending_count > 1 {
            t!(
                "agent_ui.thread_view.awaiting_confirmation_count",
                count = pending_count
            )
            .into()
        } else {
    ...
    fn render_context_compaction(
        ...
        let header_label = match &compaction.status {
            ...
            acp_thread::ContextCompactionStatus::Failed => {
                t!("agent_ui.thread_view.compaction_failed")
            }
            acp_thread::ContextCompactionStatus::Canceled => {
                t!("agent_ui.thread_view.compaction_canceled")
            }
            acp_thread::ContextCompactionStatus::Other(_) => {
                t!("agent_ui.thread_view.compaction_other")
            }
        };
        ...
        h_flex()
            ...
            .child(
                h_flex()
                    ...
                    .child(
                        Button::new("reject-all-changes", t!("agent_ui.thread_view.reject_all"))
                            .label_size(LabelSize::Small)
                            .disabled(pending_edits)
        ...
        let tooltip = match (settings_sandbox, thread_sandbox) {
            ...
            (
                ...
                let thread = (!sandbox_policy_grants_nothing(&thread)).then(|| {
                    sandbox_section(
                        t!("agent_ui.thread_view.allowed_for_thread"),
                        &thread,
                        false,
                    )
                });
                ...
                PopoverMenu::new("fast-mode-warning")
                    ...
                    .menu(move |window, cx| {
                        ...
                        Some(ContextMenu::build(window, cx, move |menu, _window, _cx| {
                            ...
                            .separator()
                            .item(
                                ContextMenuEntry::new(t!("agent_ui.thread_view.enable_now"))
                                    .handler({
                                        let weak_self = weak_self.clone();
                                        move |_window, cx| {
                                            weak_self
            ...
            move |_, cx| {
                let mut content = v_flex().gap_1().child(
                    h_flex()
                        ...
                        .justify_between()
                        .child(Label::new(t!(
                            "agent_ui.thread_view.change_thinking_effort"
                        )))
    ...
    fn render_follow_toggle(&self, cx: &mut Context<Self>) -> impl IntoElement {
        ...
        let tooltip_label = if following {
            if self.agent_id.as_ref() == agent::ZED_AGENT_ID.as_ref() {
                t!(
                    "agent_ui.thread_view.stop_following_the",
                    agent = self.agent_id
                )
            } else {
...
fn sandbox_section(
    ...
    if show_empty || !write_empty {
        section = section.group(
            SandboxGroup::new(t!("agent_ui.thread_view.write_access_group"))
                .rows(sandbox_fs_rows(&policy.fs)),
        );
    }
...
fn sandbox_network_rows(network: &SandboxNetPolicy) -> Vec<SandboxRow> {
    match network {
        SandboxNetPolicy::Unrestricted => {
            vec![SandboxRow::message(t!("agent_ui.thread_view.all_domains"))]
        }
...
impl ThreadView {
        ...
        let primary = match &entry {
            AgentThreadEntry::UserMessage(message) => {
                ...
                let agent_name = if is_subagent {
                    ...
                    AgentId(SharedString::from(t!(
                        "agent_ui.thread_view.subagents_name"
                    )))
                } else {
                    self.agent_id.clone()
                };
            ...
            v_flex()
                ...
                .child(
                    h_flex()
                        ...
                        .child(Divider::horizontal())
                        .tooltip(Tooltip::text(t!(
                            "agent_ui.thread_view.subagent_output_tooltip"
                        ))),
                )
                .child(primary)
                .into_any_element()
    ...
    fn render_thread_controls(
        ...
        let copy_response_button = copy_response_index.map(|response_index| {
            IconButton::new(("copy_agent_response", entry_ix), IconName::Copy)
                ...
                .icon_color(Color::Muted)
                .tooltip(Tooltip::text(t!(
                    "agent_ui.thread_view.copy_agent_response"
                )))
                .on_click(cx.listener(move |this, _, _, cx| {
                    let entries = this.thread.read(cx).entries();
                            ...
                            IconButton::new("feedback-thumbs-up", IconName::ThumbsUp)
                                ...
                                .tooltip({
                                    ...
                                    move |window, cx| match feedback {
                                        Some(ThreadFeedback::Positive) => Tooltip::text(t!(
                                            "agent_ui.thread_view.thanks_feedback"
                                        ))(
                                            window, cx
                                        ),
                                        _ => Tooltip::with_meta(
                ...
                ContextMenu::build(window, cx, move |menu, _, cx| {
                    ...
                    menu.when_some(focus, |menu, focus| menu.context(focus))
                        ...
                        .when_some(selected_markdown, |menu, selected_markdown| {
                            menu.entry(
                                t!("agent_ui.thread_view.copy_as_markdown"),
    ...
    fn render_sandbox_authorization_details(
        ...
        let write_section = has_write.then(|| {
            ...
            paths.sort_by(|a, b| a.requested.cmp(&b.requested));
    ...
    fn sandbox_confusable_findings(
        ...
        for path in &details.write_paths {
            let display = path.requested.display().to_string();
        ...
        v_flex()
            ...
            .when_some(thread_view, |this, thread_view| {
                ...
                let fullscreen_toggle = h_flex()
                    ...
                    .tooltip(Tooltip::text(t!(
                        "agent_ui.thread_view.make_subagent_full_screen"
                    )))
    ...
    pub(crate) fn render_thread_error(
        ...
        let callout = match self.thread_error.as_ref()? {
            ...
            ThreadError::RateLimitExceeded { provider } => self.render_error_callout(
                ...
                t!(
                    "agent_ui.thread_view.rate_limit_message",
                    provider = provider
                )
                .into(),
                true,
            ...
            ThreadError::ServerOverloaded { provider } => self.render_error_callout(
                ...
                t!(
                    "agent_ui.thread_view.provider_unavailable_message",
                    provider = provider
                )
                .into(),
                true,
            ...
            ThreadError::NoCredentials { provider } => {
                let message = Self::provider_by_name(provider, cx)
                    ...
                    .unwrap_or_else(|| {
                        t!(
                            "agent_ui.thread_view.no_credentials_message",
                            provider = provider
                        )
                        .into()
                    });
            ...
            ThreadError::StreamError { provider } => self.render_error_callout(
                ...
                t!(
                    "agent_ui.thread_view.connection_interrupted_message",
                    provider = provider
                )
                .into(),
                true,
            ...
            ThreadError::AuthenticationFailed { provider } => {
                let message = Self::provider_by_name(provider, cx)
                    ...
                    .unwrap_or_else(|| {
                        t!(
                            "agent_ui.thread_view.authentication_failed_message",
                            provider = provider
                        )
                        .into()
                    });
            ...
            ThreadError::ApiError { provider } => self.render_error_callout(
                ...
                t!(
                    "agent_ui.thread_view.api_error_message",
                    provider = provider
                )
                .into(),
                true,
    ...
    fn open_model_selector_button(&self, cx: &mut Context<Self>) -> impl IntoElement {
        Button::new(
            "open-model-selector",
            t!("agent_ui.thread_view.select_model"),
        )
        .label_size(LabelSize::Small)
        .style(ButtonStyle::Filled)
    ...
    fn render_codex_windows_warning(&self, cx: &mut Context<Self>) -> Callout {
        Callout::new()
            ...
            .actions_slot(
                Button::new("open-wsl-modal", t!("agent_ui.thread_view.open_in_wsl")).on_click(
                    cx.listener({
                        move |_, _, _window, cx| {
                            #[cfg(windows)]
    ...
    fn render_external_source_prompt_warning(&self, cx: &mut Context<Self>) -> Callout {
        Callout::new()
            ...
            .description(t!("agent_ui.thread_view.review_before_sending_description"))
            .dismiss_action(
