...
use gpui::{
    ...
    MouseDownEvent, Pixels, Point, PromptButton, PromptLevel, ScrollStrategy, Subscription, Task,
    ...
    ContextMenu::build(window, cx, |context_menu, _, _| {
        context_menu
            ...
            .action_disabled_when(
                ...
                t!("git_ui.common.stage_all"),
                StageAll.boxed_clone(),
            )
            .action_disabled_when(
                !has_staged_changes,
                t!("git_ui.common.unstage_all"),
                UnstageAll.boxed_clone(),
            )
            .action_disabled_when(
                !has_staged_tracked_changes,
                t!("git_ui.common.restore_all_changes"),
                RestoreTrackedFiles.boxed_clone(),
            )
            .separator()
            .action_disabled_when(
                !(has_new_changes || has_tracked_changes),
                t!("git_ui.git_panel.stash_all"),
            ...
            .when(group_by == GitPanelGroupBy::Status, |context_menu| {
                context_menu.action_disabled_when(
                    !has_tracked_changes,
                    t!("git_ui.git_panel.stash_tracked"),
            ...
            .when(group_by == GitPanelGroupBy::Staging, |context_menu| {
                context_menu.action_disabled_when(
                    !has_staged_changes,
                    t!("git_ui.git_panel.stash_staged"),
                    StashStaged.boxed_clone(),
                )
            })
            .action_disabled_when(
                !has_stash_items,
                t!("git_ui.git_panel.stash_pop"),
                StashPop.boxed_clone(),
            )
            .action(
                t!("git_ui.common.view_stash"),
                zed_actions::git::ViewStash.boxed_clone(),
            )
            .when(include_copy_paths, |context_menu| {
                context_menu
                    .separator()
                    .action(t!("git_ui.git_panel.copy_path"), CopyPath.boxed_clone())
                    .action(
                        t!("git_ui.git_panel.copy_relative_path"),
            ...
            .action_disabled_when(
                !has_staged_tracked_changes,
                t!("git_ui.git_panel.discard_tracked_changes"),
                RestoreTrackedFiles.boxed_clone(),
            )
            .action_disabled_when(
                !has_new_changes,
                t!("git_ui.git_panel.trash_untracked_files"),
...
fn git_panel_view_options_menu(
    ...
    ContextMenu::build_persistent(window, cx, move |context_menu, _, _| {
        ...
        context_menu
            .context(focus_handle.clone())
            .header(t!("git_ui.git_panel.view_header"))
            .item({
                let view_options_menu_state = view_options_menu_state.clone();
                ContextMenuEntry::new(t!("git_ui.git_panel.view_list"))
            ...
            .item({
                let view_options_menu_state = view_options_menu_state.clone();
                ContextMenuEntry::new(t!("git_ui.git_panel.view_tree"))
                    .toggle(IconPosition::End, state.tree_view)
                    .handler(move |window, cx| {
                        if !state.tree_view {
                            view_options_menu_state.set(GitPanelViewOptionsMenuState {
            ...
            .when(!state.tree_view, |this| {
                this.separator()
                    .header(t!("git_ui.git_panel.sort_by_header"))
                    .item({
                        let view_options_menu_state = view_options_menu_state.clone();
                        ContextMenuEntry::new(t!("git_ui.git_panel.sort_by_path"))
                    ...
                    .item({
                        let view_options_menu_state = view_options_menu_state.clone();
                        ContextMenuEntry::new(t!("git_ui.git_panel.sort_by_name"))
            ...
            .separator()
            .header(t!("git_ui.git_panel.group_by_header"))
            .item({
                let view_options_menu_state = view_options_menu_state.clone();
                ContextMenuEntry::new(t!("git_ui.git_panel.group_by_none"))
            ...
            .item({
                let view_options_menu_state = view_options_menu_state.clone();
                ContextMenuEntry::new(t!("git_ui.git_panel.group_by_tracked_untracked"))
            ...
            .item({
                let view_options_menu_state = view_options_menu_state.clone();
                ContextMenuEntry::new(t!("git_ui.git_panel.group_by_staged_unstaged"))
                    .toggle(
                        IconPosition::End,
...
impl GitHeaderEntry {
    ...
    pub fn title(&self) -> String {
        match self.header {
            Section::Conflict => t!("git_ui.git_panel.section_conflicts"),
            Section::Tracked => t!("git_ui.git_panel.section_tracked"),
            Section::New => t!("git_ui.git_panel.section_untracked"),
...
pub(crate) fn commit_message_editor(
    ...
    let placeholder =
        placeholder.unwrap_or_else(|| t!("git_ui.git_panel.enter_commit_message").into());
...
impl GitPanel {
    ...
    fn revert_entries(
        ...
        maybe!({
            ...
            let prompt = if skip_prompt {
                ...
                let entry_display = entry.repo_path.display(path_style);
                let file = MarkdownInlineCode(
                    entry
                        .repo_path
                        .file_name()
                        .unwrap_or(entry_display.as_ref()),
                );
                let (message, confirm_text) = if entry.status.is_deleted() {
                    (
                        t!("git_ui.git_panel.restore_file_confirm", file = file),
                        t!("git_ui.git_panel.restore_file"),
                    )
                } else if entry.status.is_created() {
                    (
                        t!("git_ui.git_panel.trash_file_confirm", file = file),
                        t!("git_ui.git_panel.trash"),
                    )
                } else {
                    (
                        t!("git_ui.git_panel.discard_changes_confirm", file = file),
                ...
                let (message, confirm_text) = match (tracked.len(), untracked.len()) {
                    ...
                    (tracked_count, 0) => (
                        t!(
                            "git_ui.git_panel.discard_count_files",
                            count = tracked_count
                        ),
                        t!("git_ui.git_panel.discard"),
                    ),
                    (0, untracked_count) => (
                        t!(
                            "git_ui.git_panel.trash_count_files",
                    ...
                    (tracked_count, untracked_count) => (
                        t!(
                            "git_ui.git_panel.discard_trash_count_files",
                            count1 = tracked_count,
                            count2 = untracked_count
                        ),
                        t!("git_ui.git_panel.discard_and_trash"),
                    ),
                };
            ...
            .detach_and_prompt_err(
                &t!("git_ui.git_panel.revert_failed"),
                window,
                cx,
                |e, _, _| Some(format!("{e}")),
            );
    ...
    fn restore_tracked_files(
        ...
        let mut details = entries
            ...
            .join("\n");
        if entries.len() > 5 {
            details.push_str(&t!("git_ui.git_panel.and_more", count = entries.len() - 5))
        }
        ...
        let prompt = prompt(
            &t!("git_ui.git_panel.discard_changes_to_files"),
            Some(&details),
            window,
            cx,
        );
    ...
    fn clean_all(&mut self, _: &TrashUntrackedFiles, window: &mut Window, cx: &mut Context<Self>) {
        ...
        if to_delete.len() > 5 {
            details.push_str(&t!(
                "git_ui.git_panel.and_more",
                count = to_delete.len() - 5
            ))
        }

        let prompt = prompt(
            &t!("git_ui.git_panel.trash_these_files"),
            Some(&details),
            window,
            cx,
        );
        ...
        .detach_and_prompt_err(
            &t!("git_ui.git_panel.trash_files_failed"),
            window,
            cx,
            |e, _, _| Some(format!("{e}")),
        );
    ...
    pub(crate) fn commit_changes(
        ...
        if self.has_unstaged_conflicts() {
            error_spawn(
                &t!("git_ui.git_panel.conflicts_must_stage_before_commit"),
                window,
                cx,
            );
            return;
        ...
        let task = if self.has_staged_changes() {
            ...
            if changed_files.is_empty() && !options.amend {
                error_spawn(&t!("git_ui.git_panel.no_changes_to_commit"), window, cx);
                return;
            }
    ...
    fn check_for_pushed_commits(
        ...
        async move {
            ...
            if pushed_to.is_empty() {
                ...
                let detail = t!(
                    "git_ui.git_panel.uncommit_already_pushed",
                    remotes = pushed_to.into_iter().join(", ")
                );
                let result = cx
                    .update(|window, cx| {
                        prompt(
                            &t!("git_ui.git_panel.are_you_sure"),
                            Some(&detail),
                            window,
                            cx,
                        )
            ...
            let selection = cx
                .update(|window, cx| {
                    picker_prompt::prompt(
                        &t!("git_ui.git_panel.pick_remote_to_fetch"),
    ...
    pub(crate) fn git_init(&mut self, window: &mut Window, cx: &mut Context<Self>) {
        ...
        let worktree = if worktrees.len() == 1 {
            ...
            let result = window.prompt(
                PromptLevel::Warning,
                &t!("git_ui.git_panel.init_repo_failed"),
                Some(&t!("git_ui.git_panel.open_directory_first")),
                &["OK"],
                cx,
            );
            cx.background_executor()
            ...
            let prompt = picker_prompt::prompt(
                &t!("git_ui.git_panel.pick_init_directory"),
    ...
    fn get_remote(
        ...
        async move {
            ...
            let selection = cx
                .update(|window, cx| {
                    picker_prompt::prompt(
                        &t!("git_ui.git_panel.pick_remote_to_push"),
                    ...
                    workspace.update(cx, |workspace, cx| {
                        ...
                        workspace.show_toast(
                            workspace::Toast::new(
                                ...
                                t!("git_ui.git_panel.no_active_repository"),
                            )
                            .autohide(),
                            cx,
                        );
            ...
            let status_toast = StatusToast::new(message, cx, move |this, _cx| {
                ...
                match (style, is_push) {
                    ...
                    (Toast | ToastWithLog { .. }, true) => {
                        ...
                        // button is pressed.
                        this.action(
                            t!("git_ui.git_panel.create_pull_request"),
                            move |window, cx| {
                    ...
                    (ToastWithLog { output }, false) => {
                        this.action(t!("git_ui.git_panel.view_log"), move |window, cx| {
    ...
    fn render_view_options_menu(&self, id: impl Into<ElementId>) -> impl IntoElement {
        ...
        PopoverMenu::new(id.into())
            .trigger_with_tooltip(
                ...
                Tooltip::text(t!("git_ui.git_panel.view_options")),
            )
            .menu(move |window, cx| {
    ...
    pub(crate) fn render_generate_commit_message_button(
        ...
        if self.generate_commit_message_task.is_some() {
            return Some(
                h_flex()
                    ...
                    .child(
                        Label::new(t!("git_ui.git_panel.generating_commit"))
        ...
        let button = if can_commit && has_commit_model_configuration_error {
            ...
            button.tooltip(move |_window, cx| {
                if !can_commit {
                    Tooltip::simple(t!("git_ui.git_panel.no_changes_to_commit"), cx)
                } else {
                    Tooltip::for_action_in(
                        t!("git_ui.git_panel.generate_commit_message"),
    ...
    pub(crate) fn render_co_authors(&self, cx: &Context<Self>) -> Option<AnyElement> {
        ...
        let (tooltip_label, icon) = if self.add_coauthors {
            (
                t!("git_ui.git_panel.remove_co_authored_by"),
                IconName::Person,
            )
        } else {
            (
                t!("git_ui.git_panel.add_co_authored_by"),
                IconName::UserCheck,
            )
        };
                    ...
                    ButtonLike::new("diff-button")
                        .child(
                            h_flex()
                                ...
                                .child(
                                    Label::new(t!("git_ui.git_panel.view_diff"))
                                        .size(LabelSize::Small)
                                        .color(Color::Muted),
                                )
                                .when(
        ...
        let vertical_buttons = v_flex()
            ...
            .child(
                IconButton::new("expand-commit-editor", IconName::MaximizeAlt)
                    ...
                    .tooltip({
                        move |_window, cx| {
                            Tooltip::for_action_in(
                                t!("git_ui.git_panel.open_commit_modal"),
            ...
            .child({
                let (icon, label) = if self.commit_editor_expanded {
                    (
                        IconName::Minimize,
                        t!("git_ui.git_panel.collapse_commit_editor"),
                    )
                } else {
                    (
                        IconName::Maximize,
                        t!("git_ui.git_panel.expand_commit_editor"),
                    )
                };
                let focus_handle = self.focus_handle.clone();
        ...
        h_flex()
            ...
            .child(
                div()
                    ...
                    .max_w(relative(0.85))
                    .child(
                        Label::new(t!("git_ui.git_panel.amend_will_update_last_commit"))
                            .size(LabelSize::Small)
                            .truncate(),
                    ),
            )
            .child(
                Button::new("cancel", t!("git_ui.common.cancel"))
                    ...
                    h_flex()
                        ...
                        .child(
                            IconButton::new("git-graph-button", IconName::GitGraph)
                                ...
                                .tooltip(|_window, cx| {
                                    Tooltip::for_action(
                                        t!("git_ui.git_panel.open_git_graph"),
    ...
    fn render_tab_bar(&self, cx: &mut Context<Self>) -> impl IntoElement {
        ...
        let tab = |id: ElementId,
            ...
            h_flex()
                ...
                .tooltip(Tooltip::for_action_title_in(
                    t!("git_ui.git_panel.toggle_tab", tab = label),
        ...
        h_flex()
            ...
            .child(tab(
                ...
                active_tab == GitPanelTab::Changes,
                true,
                t!("git_ui.git_panel.tab_changes").into(),
                GitPanelTab::Changes,
            ...
            .child(tab(
                ...
                active_tab != GitPanelTab::Changes,
                false,
                t!("git_ui.git_panel.tab_history").into(),
                GitPanelTab::History,
    ...
    fn render_history_tab(&self, window: &mut Window, cx: &mut Context<Self>) -> impl IntoElement {
        v_flex().flex_1().size_full().overflow_hidden().map(|this| {
            ...
            match &self.commit_history {
                _ if !has_repo => this.child(Self::render_history_placeholder(t!(
                    "git_ui.git_panel.no_repository_found"
                ))),
                CommitHistory::Error(_) => this.child(Self::render_history_placeholder(t!(
                    "git_ui.git_panel.history_load_failed"
                ))),
                CommitHistory::Loading => this.child(Self::render_history_placeholder(t!(
                    "git_ui.git_panel.loading_commit_history"
                ))),
                CommitHistory::Loaded(entries) if entries.is_empty() => this.child(
                    Self::render_history_placeholder(t!("git_ui.git_panel.no_commits_yet")),
                ),
                CommitHistory::Loaded(_) => match self.render_commit_history(window, cx) {
                    Some(history) => this.child(history),
                    None => this.child(Self::render_history_placeholder(t!(
                        "git_ui.git_panel.commits_load_failed"
                    ))),
                },
            }
        })
    ...
    fn render_history_placeholder(message: impl Into<SharedString>) -> impl IntoElement {
        h_flex()
            .flex_1()
            .justify_center()
                                    ...
                                    v_flex()
                                        ...
                                        .when(!has_context_menu, |this| {
                                            this.tooltip(move |_, cx| {
                                                ...
                                                Tooltip::with_meta(
                                                    t!("git_ui.common.view_commit_diff"),
                                                    None,
                                                    description,
                                                    cx,
                                                )
    ...
    fn render_no_changes_ui(&self, cx: &Context<Self>) -> AnyElement {
        ...
        v_flex()
            .gap_1()
            .items_center()
            .child(Label::new(t!("git_ui.git_panel.no_changes_to_commit")).color(Color::Muted))
            .when(show_branch_diff, |this| {
                this.child(
                    Button::new("view_branch_diff", t!("git_ui.git_panel.view_branch_diff"))
                        .label_size(LabelSize::Small)
    ...
    fn render_unsafe_repo_ui(
        ...
        let message = t!(
            "git_ui.git_panel.dubious_ownership_message",
        ...
        v_flex()
                ...
                .child(
                    h_flex()
                        ...
                        .gap_1()
                        .child(
                            Button::new("trust_directory", t!("git_ui.git_panel.trust_directory"))
                            .label_size(LabelSize::Small)
                            .layer(ElevationIndex::ModalSurface)
                    ...
                    .child(
                        Button::new("learn_more", t!("git_ui.common.learn_more"))
                            .label_size(LabelSize::Small)
                            .style(ButtonStyle::Outlined)
    ...
    fn render_uninitialized_ui(&self, cx: &mut Context<Self>) -> AnyElement {
        ...
        if worktree_count > 0 && self.active_repository.is_none() {
            v_flex()
                .gap_1()
                .items_center()
                .child(Label::new(t!("git_ui.git_panel.no_git_repositories")).color(Color::Muted))
                .child(
                    Button::new(
                        "initialize_repository",
                        t!("git_ui.git_panel.initialize_repository"),
                    )
                    .label_size(LabelSize::Small)
                    .style(ButtonStyle::Outlined)
                    .tooltip(Tooltip::for_action_title_in(
        ...
        h_flex()
            ...
            .child(if section_is_empty {
                ...
                let tooltip_label = if all_conflicts_resolved {
                    ...
                    match stage_intent {
                        StageIntent::Stage => Some(t!("git_ui.common.stage_all")),
                        StageIntent::Unstage => Some(t!("git_ui.common.unstage_all")),
                        StageIntent::Toggle => None,
                    }
                };
                if let Some(label) = tooltip_label {
                    checkbox
                        .tooltip(move |_window, cx| Tooltip::simple(label.clone(), cx))
                        .into_any_element()
                } else {
    ...
    fn render_empty_section(&self, section: Section) -> AnyElement {
        let message = match section {
            ...
            Section::Unstaged => t!("git_ui.git_panel.no_unstaged_changes"),
            _ => t!("git_ui.common.no_changes"),
        };
        h_flex()
            .h(self.list_item_height())
            .w_full()
    ...
    fn build_context_menu(
        ...
        let stage_title = match (target_kind, will_unstage) {
            (SelectionTargetKind::Directory, true) => t!("git_ui.git_panel.unstage_folder"),
            (SelectionTargetKind::Directory, false) => t!("git_ui.git_panel.stage_folder"),
            (SelectionTargetKind::File, true) => t!("git_ui.git_panel.unstage_file"),
            (SelectionTargetKind::File, false) => t!("git_ui.git_panel.stage_file"),
            (SelectionTargetKind::Multiple, true) => t!(
                "git_ui.git_panel.unstage_count_files",
                count = file_count,
                files = file_label
            )
            .into(),
            (SelectionTargetKind::Multiple, false) => t!(
                "git_ui.git_panel.stage_count_files",
                count = file_count,
                files = file_label
            )
            .into(),
        ...
        let restore_title = match (target_kind, all_created, all_deleted) {
            (SelectionTargetKind::Directory, true, _) => t!("git_ui.git_panel.trash_folder"),
            (SelectionTargetKind::Directory, false, _) => t!("git_ui.git_panel.discard_changes"),
            (SelectionTargetKind::File, true, _) => t!("git_ui.git_panel.trash_file"),
            (SelectionTargetKind::File, _, true) => t!("git_ui.git_panel.restore_file"),
            (SelectionTargetKind::File, _, _) => t!("git_ui.git_panel.discard_changes"),
            (SelectionTargetKind::Multiple, true, _) => t!(
                "git_ui.git_panel.trash_count_files",
                count = file_count,
                files = file_label
            )
            .into(),
            (SelectionTargetKind::Multiple, _, true) => t!(
                "git_ui.git_panel.restore_count_files",
                count = file_count,
                files = file_label
            )
            .into(),
            (SelectionTargetKind::Multiple, _, _) => t!(
                "git_ui.git_panel.discard_count_files",
                count = file_count,
                files = file_label
            )
        ...
        ContextMenu::build(window, cx, |context_menu, _, _| {
            context_menu
                ...
                .separator()
                .action(
                    t!("git_ui.git_panel.view_unstaged_changes"),
                    ViewUnstagedChanges.boxed_clone(),
                )
                .action(
                    t!("git_ui.git_panel.view_staged_changes"),
                    ViewStagedChanges.boxed_clone(),
                )
                .separator()
                .action_disabled_when(
                    !all_created || is_bulk,
                    t!("git_ui.git_panel.add_to_gitignore"),
                    AddToGitignore.boxed_clone(),
                )
                .action_disabled_when(
                    !all_created || is_bulk,
                    t!("git_ui.git_panel.add_to_git_info_exclude"),
                ...
                .when(is_file, |context_menu| {
                    context_menu
                        .separator()
                        .action(
                            t!("git_ui.git_panel.open_diff"),
                            menu::Confirm.boxed_clone(),
                        )
                        .action(
                            t!("git_ui.git_panel.open_file_diff"),
                            menu::SecondaryConfirm.boxed_clone(),
                        )
                        .action(t!("git_ui.git_panel.view_file"), ViewFile.boxed_clone())
                })
                .when(is_file && !all_created, |context_menu| {
                    context_menu.separator().action(
                        t!("git_ui.git_panel.view_file_history"),
                        ...
                        Checkbox::new(checkbox_id, toggle_state)
                            ...
                            .tooltip(move |_window, cx| {
                                if resolved_conflict {
                                    Tooltip::simple(t!("git_ui.git_panel.conflict_resolved"), cx)
                                } else {
                ...
                div()
                    ...
                    .child(
                        Checkbox::new(checkbox_id, toggle_state)
                            ...
                            .tooltip(move |_window, cx| {
                                if resolved_conflict {
                                    Tooltip::simple(t!("git_ui.git_panel.conflicts_resolved"), cx)
                                } else {
                                    let action = stage_intent.label(|| stage_status);
                                    Tooltip::simple(
                                        t!("git_ui.git_panel.action_folder", action = action),
                                        cx,
                                    )
                                }
                            }),
    ...
    fn render(&mut self, _window: &mut Window, cx: &mut Context<Self>) -> impl IntoElement {
        ui::tooltip_container(cx, |container, _cx| {
            container
                .gap_1p5()
                .child(Label::new(t!(
                    "git_ui.git_panel.configure_llm_provider_hint",
                )))
                .child(
                    h_flex()
                        .gap_1()
                        .child(
                            Button::new(
                                ...
                                t!("git_ui.git_panel.configure_provider"),
                            )
                            .style(ButtonStyle::Filled)
                            .layer(ElevationIndex::ModalSurface)
                            .label_size(LabelSize::Small)
                        ...
                        .child(
                            Button::new("llm-provider-docs", t!("git_ui.git_panel.see_docs"))
            ...
            .trigger_with_tooltip(
                ...
                move |_, cx| {
                    if single_repo {
                        ...
                        Tooltip::simple(t!("git_ui.git_panel.switch_active_repository"), cx)
                    }
                },
            )
...
pub(crate) fn open_output(
    ...
    let editor = cx.new(|cx| {
        ...
        editor.buffer().update(cx, |buffer, cx| {
            buffer.set_title(
                t!("git_ui.git_panel.output_title", operation = operation),
                cx,
            );
        });
        ...
        cx.defer(move |cx| {
            workspace.update(cx, |workspace, cx| {
                ...
                let toast = StatusToast::new(
                    t!("git_ui.git_panel.git_action_failed", action = action),
                    cx,
                    |this, _cx| {
                        this.icon(
