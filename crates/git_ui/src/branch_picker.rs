...
impl BranchFilter {
    ...
    fn label(self) -> String {
        match self {
            Self::All => t!("git_ui.branch_picker.all_branches"),
    ...
    fn render(&mut self, _: &mut Window, cx: &mut Context<Self>) -> impl IntoElement {
        ...
        if force_delete {
            Tooltip::for_action_in(
                t!("git_ui.branch_picker.force_delete_branch"),
...
impl BranchListDelegate {
    ...
    fn branch_filter_tooltip(&self) -> impl Fn(&mut Window, &mut App) -> gpui::AnyView + 'static {
        ...
        move |_, cx| {
            Tooltip::for_action_in(
                t!("git_ui.branch_picker.filter_branches"),
...
impl PickerDelegate for BranchListDelegate {
    ...
    fn placeholder_text(&self, _window: &mut Window, _cx: &mut App) -> Arc<str> {
        match self.state {
            PickerState::List | PickerState::NewRemote | PickerState::NewBranch => {
                t!("git_ui.branch_picker.placeholder_switch_or_create")
            }
            PickerState::CreateRemote(_) => t!("git_ui.branch_picker.placeholder_remote_name"),
        }
        .into()
    }
    ...
    fn update_matches(
        ...
        cx.spawn_in(window, async move |picker, cx| {
            let branch_matches_filter = |branch: &Branch| match branch_filter {
                ...
                BranchFilter::Local => !branch.is_remote(),
            ...
            picker
                .update(cx, |picker, _| {
                    ...
                    if !picker.delegate.is_select_only()
                        && !query.is_empty()
                        && !all_branches.iter().any(|branch| branch.name() == query)
                    {
                        let query = normalize_branch_name(&query);
    ...
    fn confirm(&mut self, secondary: bool, window: &mut Window, cx: &mut Context<Picker<Self>>) {
        ...
        match entry {
            Entry::Branch { branch, .. } => {
                ...
                .detach_and_prompt_err(
                    &t!("git_ui.branch_picker.change_branch_failed"),
                    window,
                    cx,
                    |_, _, _| None,
                );
    ...
    fn render_match(
        ...
        let entry_icon = match entry {
            ...
            Entry::Branch { branch, .. } => {
                if is_checked_branch {
                    ...
                    branch
                        .remote_name()
                        .and_then(|remote_name| self.remote_provider_icons.get(remote_name))
                        .copied()
        ...
        let entry_title = match entry {
            Entry::NewUrl { .. } => Label::new(t!("git_ui.branch_picker.create_remote_repository"))
                .single_line()
                .truncate()
                .into_any_element(),
            Entry::NewBranch { name } => {
                        ...
                        v_flex()
                            ...
                            .child({
                                let message = match entry {
                                    Entry::NewUrl { url } => {
                                        t!("git_ui.branch_picker.based_off", base = url)
                                    }
                                ...
                                if matches!(entry, Entry::Branch { .. }) {
                                    ...
                                    h_flex()
                                        ...
                                        .when_some(subject, |this, subj| {
                                            this.when(has_commit, |this| this.child(dot())).child(
                                        ...
                                        Tooltip::element(move |_, _| {
                                            v_flex()
                                                ...
                                                .when(is_select_only && is_checked, |this| {
                                                    this.child(
                                                        Label::new(t!(
                                                            "git_ui.branch_picker.selected_branch"
                                                        ))
                                                        .size(LabelSize::Small)
                                                        .color(Color::Muted),
                                                    )
                                                ...
                                                .when(is_head, |this| {
                                                    this.child(
                                                        Label::new(t!(
                                                            "git_ui.branch_picker.current_branch"
                                                        ))
                                                        .size(LabelSize::Small)
                                                        .color(Color::Muted),
                                                    )
        ...
        let section_header = (self.branch_filter == BranchFilter::All)
            ...
            .and_then(|branch| {
                ...
                starts_section.then(|| {
                    if branch.is_remote() {
                        (t!("git_ui.branch_picker.remote_branches"), ix != 0)
                    } else {
                        (t!("git_ui.branch_picker.local_branches"), false)
                    }
                })
            });
        ...
        match self.state {
            PickerState::List => {
                ...
                let branch_from_default_button = self
                    ...
                    .map(|default_branch| {
                        let button_label = t!(
                            "git_ui.branch_picker.create_new_from",
                ...
                let delete_and_select_btns = h_flex()
                    ...
                    .when(
                        ...
                        |this| {
                            this.child(
                                Button::new("delete-branch", t!("git_ui.common.delete"))
                                    .key_binding(
                                        KeyBinding::for_action_in(
                    ...
                    footer_container()
                        ...
                        .map(|this| match branch_from_default_button {
                            Some(button) => this.child(button).child(
                                Button::new("create", t!("git_ui.common.create"))
                                    .key_binding(
                                        KeyBinding::for_action_in(
                                            &menu::Confirm,
                                            &focus_handle,
...
mod tests {
    ...
    async fn test_update_branch_matches_with_query(cx: &mut TestAppContext) {
        ...
        for branch_filter in [BranchFilter::Local, BranchFilter::Remote] {
            branch_list
                .update_in(cx, |branch_list, window, cx| {
                    branch_list.picker.update(cx, |picker, cx| {
                        picker.delegate.branch_filter = branch_filter;
                        picker
                            .delegate
                            .update_matches(String::from("missing-branch"), window, cx)
                    })
                })
                .await;
            cx.run_until_parked();
            ...
            branch_list.update_in(cx, |branch_list, window, cx| {
                branch_list.picker.update(cx, |picker, cx| {
                    assert!(
                        picker.delegate.render_header(window, cx).is_none(),
                        "create-only results should not be shown under a branch section header"
                    );
                    assert!(matches!(
                        picker.delegate.matches.as_slice(),
                        [Entry::NewBranch { .. }]
                    ));
                })
            });
        ...
        branch_list
            .update_in(cx, |branch_list, window, cx| {
                branch_list.picker.update(cx, |picker, cx| {
                    ...
                    picker
                        .delegate
                        .update_matches(String::from("feature-ui"), window, cx)
                })
            })
            .await;
        cx.run_until_parked();
        ...
        branch_list.update(cx, |branch_list, cx| {
            branch_list.picker.update(cx, |picker, _cx| {
                assert!(
                    ...
                    "an exact branch hidden by the active filter should not be offered for creation"
                );
            })
        });
        ...
        branch_list
            .update_in(cx, |branch_list, window, cx| {
                branch_list.picker.update(cx, |picker, cx| {
                    picker
                        .delegate
                        .update_matches(String::from("feature-u"), window, cx)
                })
            })
            .await;
        cx.run_until_parked();
